...
def _clamp_menu_rect(rect: QRect, avail: QRect) -> QRect:
    ...
    from .menu_animation import _clamp_menu_rect as _impl
    return _impl(rect, avail)
...
def animate_context_menu_to(
    ...
    *,
    ...
    from .menu_animation import animate_context_menu_to as _impl
    return _impl(menu, target, duration_ms=duration_ms)
...
def pick_context_menu_position(
    ...
    实现已抽到 ``pet/menu_animation.py``（轻依赖、避免 window 全家冷启动）；
    本函数保持位置与签名不变作兼容 re-export。
    """
    from .menu_animation import pick_context_menu_position as _impl
    return _impl(pet_rect, menu_size, submenu_width, avail, margin=margin)
