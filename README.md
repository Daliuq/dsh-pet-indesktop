...
>
> **开发版 main（未发布，v4.2.1 候选）**：已并入 PR #97（事件汇报概率门 + 事件链路语义对齐 + issue #95：Windows node/pnpm 解析不再写死）、#100 / #102（启动即按配置装配可选服务，issue #99）、#101（边缘探头会话期间禁止位移）。本机全量套件 1805 passed / 8 skipped，新增内容见下方 [最近修复与变更记录](#最近修复与变更记录)。
>
...
4. **`window.py` 行数预算**：当前预算 `4385` 行（红线常量见 `tests/test_architecture.py`，实测 4294 行，2026-09-11 校准；含 klxxya 的 #101 探头位移闸门与 #102 启动装配注释）。预算只随实测校准，**不靠压缩行宽/合并语句硬塞**；确需上调要在 PR 说明理由。
...
- **开发版 main（未发布，v4.2.1 候选，2026-09-11）**：v4.2.0 之后又并入 PR #97（事件汇报概率门 + 事件链路语义对齐 + issue #95：Windows node/pnpm 解析不再写死）、#100 / #102（启动即按配置装配可选服务，issue #99）、#101（边缘探头会话期间禁止位移）——详见下方 [最近修复与变更记录](#最近修复与变更记录)。
